...
RSS_SOURCES = [
    ...
    ("https://nordicstartupnews.com/feed/",       "Nordic Startup News"),
...
def analyse_article_llm(title: str, summary: str) -> dict | None:
    """
    Single Gemini call replacing three separate calls for:
      1. relevance check
      2. company name extraction
      3. data-role hiring assessment

    Returns dict with keys: relevant (bool), company (str), data_roles (bool).
    Returns None if Gemini unavailable or response unparseable.
    """
    if _gemini_client is None:
        return None
...
<div style="max-width:1060px;margin:auto;background:#fff;border-radius:12px;
  ...
  <div style="background:#f9fafb;padding:16px 32px;font-size:12px;
    ...
    🤖 Nordic Startup Funding Agent v9 ({mode}) &nbsp;·&nbsp;
...
def main() -> None:
    print(f"🚀 Agent v9 — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
